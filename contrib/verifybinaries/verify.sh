...
###   youngseokcoincore.org and youngseokcoin.org and compares them.
...
WORKINGDIR="/tmp/youngseokcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://youngseokcoincore.org"
HOST2="https://youngseokcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="youngseokcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'youngseokcoin-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "youngseokcoin.org failed to provide signature file, but youngseokcoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "youngseokcoin.org and youngseokcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Youngseokcoin Core binary release signing key installed?"
   fi
