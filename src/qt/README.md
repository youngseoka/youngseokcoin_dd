This directory contains the YoungseokcoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
### youngseokcoingui.(h/cpp)

Represents the main window of the Youngseokcoin UI.
...
ClientModel is used by the main application `youngseokcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a youngseokcoin:... link from a browser).
...
* UI elements like YoungseokcoinAmountField, which inherit from QWidget.
* `youngseokcoinstrings.cpp`: automatically generated
* `youngseokcoinunits.(h/cpp)`: YSC / mYSC / etc handling
* `callback.h`
...
* don't change `local/youngseokcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "youngseokcoin-qt" under "Run", which is where you can also add command line arguments)
