 ...
 */
static const char* YOUNGSEOKCOIN_PID_FILENAME = "youngseokcoind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", YOUNGSEOKCOIN_PID_FILENAME)));
}
...
void SetupServerArgs(NodeContext& node)
{
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", YOUNGSEOKCOIN_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/youngseokcoin/youngseokcoin>";
...
/** Sanity checks
 *  Ensure that Youngseokcoin is running in a usable environment with all
...
{
    // Make sure only a single Youngseokcoin process is using the data directory.
...
bool AppInitMain(NodeContext& node)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", YOUNGSEOKCOIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if youngseokcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if youngseokcoin is started while in a temporary directory.\n",
