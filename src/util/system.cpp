...
const char * const YOUNGSEOKCOIN_CONF_FILENAME = "youngseokcoin.conf";
 ...
 * https://github.com/youngseokcoin/youngseokcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //youngseokcoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "youngseokcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Youngseokcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Youngseokcoin
    // Mac: ~/Library/Application Support/Youngseokcoin
    // Unix: ~/.youngseokcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Youngseokcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Youngseokcoin";
#else
    // Unix
    return pathRet / ".youngseokcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", YOUNGSEOKCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Youngseokcoin Core copyright is not removed by accident
    if (copyright_devs.find("Youngseokcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Youngseokcoin Core developers";
    }
