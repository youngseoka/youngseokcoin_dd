...
class ToolWalletTest(YoungseokcoinTestFramework):
    ...
    def youngseokcoin_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/youngseokcoin-wallet' + self.config["environment"]["EXEEXT"]
        args = ['-datadir={}'.format(self.nodes[0].datadir), '-chain=%s' % self.chain] + list(args)
        return subprocess.Popen([binary] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)

    def assert_raises_tool_error(self, error, *args):
        p = self.youngseokcoin_wallet_process(*args)
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `youngseokcoin-wallet help` raises an error. Use `youngseokcoin-wallet -help`.
